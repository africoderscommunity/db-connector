...
//           await new Promise((resolve, reject) => {
//             const initProc = spawn(initdbPath, ['-D', dataPath, '-U', 'postgres'], { shell: false });
...
//           });
//         }
//         args.push('-D', dataPath, '-p', port.toString());
//       } else if (engine === 'mysql') {
...
//         username: engine === 'postgres' ? "postgres" : null,
//         status: 'running', 
//         pid: proc.pid, 
//         logs: '' 
//       };
...
//       info.note = `You can also connect as 'postgres' user (superuser created during initialization)`;
          ...
          await new Promise((resolve, reject) => {
            ...
            initProc.on('exit', code => {
              if (code === 0) {
                ...
                // Configure pg_hba.conf - use 'password' method which accepts empty passwords
                const pgHbaPath = path.join(dataPath, 'pg_hba.conf');
                const pgHbaContent = `# TYPE  DATABASE        USER            ADDRESS                 METHOD
# 'password' method allows both empty and set passwords
local   all             all                                     password
host    all             all             127.0.0.1/32            password
host    all             all             ::1/128                 password
`;
                fs.writeFileSync(pgHbaPath, pgHbaContent);
                console.log(`[DBEngine] Configured pg_hba.conf for password authentication`);
  ...
  getConnectionInfo(id) {
    ...
    if (instance.engine === 'mongo') {
      ...
      info.password = null;
      info.database = 'postgres';
      info.note = 'Connect without password: psql "postgresql://postgres:@127.0.0.1:' + instance.port + '/postgres" \n\nTo set a password, connect and run: ALTER ROLE postgres WITH PASSWORD \'yourpassword\'; \n\nThen connect with: psql "postgresql://postgres:yourpassword@127.0.0.1:' + instance.port + '/postgres"';
    } else if (instance.engine === 'mysql') {
      const mySqlUser = 'root';
      info.connectionString = `mysql://root@127.0.0.1:${instance.port}`;
      info.username = mySqlUser;
      info.password = null;
      info.database = 'mysql';
      info.note = 'Connect without password: mysql -u root -h 127.0.0.1 -P ' + instance.port + ' \n\nTo set a password, connect and run: ALTER USER \'root\'@\'localhost\' IDENTIFIED BY \'yourpassword\'; FLUSH PRIVILEGES; \n\nThen connect with: mysql -u root -p -h 127.0.0.1 -P ' + instance.port;
    }
