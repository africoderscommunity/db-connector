...
// // // https://github.com/theseus-rs/postgresql-binaries/releases/download/18.0.0/postgresql-18.0.0-aarch64-apple-darwin.tar.gz
// // // https://github.com/theseus-rs/postgresql-binaries/releases/download/18.0.0/postgresql-18.0.0-arm-unknown-linux-gnueabi.tar.gz
...
// //         console.log({pgArch})
// //         // url = `https://github.com/theseus-rs/postgresql-binaries/releases/download/18.0.0/postgresql-18.0.0-aarch64-apple-darwin.tar.gz`
...
//     console.log(`[DBEngine] Will run database processes as user: ${this.dbUser}`);

//     this.binaries = this.listBinaries();
//   }

//   /**
//    * Determine which user should run database processes
//    * - If running as root, find a suitable non-root user
...
//     console.log('[DBEngine] Running as root, finding non-root user for database processes...');

//     // Try to find a suitable non-privileged user
//     // Common candidates: _postgres, postgres, _mysql, mysql, nobody, or the user who invoked sudo
//     const candidates = [
//       process.env.SUDO_USER, // User who ran sudo
//       '_postgres',
//       'postgres',
//       '_mysql',
//       'mysql',
...
//   /**
//    * Spawn a process as the appropriate user
//    */
//   spawnAsUser(command, args, options = {}) {
//     const currentUser = os.userInfo().username;

//     // If we're the right user already, spawn normally
//     if (currentUser === this.dbUser) {
//       return spawn(command, args, options);
//     }

//     // Otherwise, use 'su' to run as the db user
//     console.log(`[DBEngine] Spawning as user ${this.dbUser}: ${command} ${args.join(' ')}`);

//     // Use 'su' to switch user and run command
//     const suArgs = [
//       '-',  // Login shell
//       this.dbUser,
//       '-c',
...
//       ...options,
//       shell: true // Need shell for su command
//     });
//   }

//   /**
//    * Fix permissions on data directory for the db user
//    */
...
//       try {
//         console.log(`[DBEngine] Fixing ownership of ${dataPath} for user ${this.dbUser}`);
...
//         if (engine === 'postgres') binPath = path.join(this.binariesDir, latest, 'bin', 'postgres');
//         if (engine === 'mysql') binPath = path.join(this.binariesDir, latest, 'bin', 'mysqld');
//         if (engine === 'redis') binPath = path.join(this.binariesDir, latest, 'src', 'redis-server');
...
//         const redisArch = process.arch === 'arm64' ? 'arm64' : 'x86_64';
...
//       if (engine === 'postgres') binaryPath = path.join(targetDir, 'bin', 'postgres');
//       if (engine === 'mysql') binaryPath = path.join(targetDir, 'bin', 'mysqld');
//       if (engine === 'redis') binaryPath = path.join(targetDir, 'src', 'redis-server');

//       // After the extraction complete log, add:
// if (engine === 'redis') {
...
//       throw new Error(`Make command failed: ${makeResult.error?.message || `Exit code ${makeResult.status}`}`);
//     }
...
//     // Fix permissions for the database user
...
//         if (engine === 'postgres') binaryPath = path.join(binDir, 'bin', 'postgres');
//         if (engine === 'mysql') binaryPath = path.join(binDir, 'bin', 'mysqld');
//         if (engine === 'redis') binaryPath = path.join(binDir, 'src', 'redis-server');
...
//         if (!fs.existsSync(pgVersionFile)) {
//           const initdbPath = path.join(path.dirname(binaryPath), 'initdb');
//           console.log(`[DBEngine] Initializing PostgreSQL at ${dataPath} as user ${this.dbUser}`);

//           // Initialize with postgres user and trust authentication (no password)
//           await new Promise((resolve, reject) => {
//             const initProc = this.spawnAsUser(initdbPath, [
//               '-D', dataPath,
//               '-U', 'postgres',
//               '--auth=trust'  // Allow connections without password initially
//             ], { shell: false });
...
//         const mysqlInitMarker = path.join(dataPath, 'mysql');
//         if (!fs.existsSync(mysqlInitMarker)) {
//           console.log(`[DBEngine] Initializing MySQL at ${dataPath} as user ${this.dbUser}`);

//           // Initialize with --initialize-insecure (no root password)
//           await new Promise((resolve, reject) => {
//             const initProc = this.spawnAsUser(binaryPath, [
//               '--initialize-insecure',
//               `--datadir=${dataPath}`
//             ], { shell: false });
...
//       console.log(`[DBEngine] Starting ${engine} as user ${this.dbUser} with args:`, args);
