...
contextBridge.exposeInMainWorld('electron', {
  db: {
    ...
    deleteTable: ({ connection, tableName }) => ipcRenderer.invoke('db:delete:table', { connection, tableName }),
    getTableStructure: (params) => ipcRenderer.invoke('db:get:structure',params),
  ...
  // createInstance: (engine, port, version) => ipcRenderer.invoke('create-instance', engine, port, version),
  // startInstance: async (id) => {
  //   await ipcRenderer.invoke('start-instance', id);
  //   return ipcRenderer.invoke('list-instances'); // refresh instances after start
  // },
  // stopInstance: async (id) => {
  //   await ipcRenderer.invoke('stop-instance', id);
  //   return ipcRenderer.invoke('list-instances'); // refresh instances after stop
  // },
...
listBinaries: () => ipcRenderer.invoke('list-binaries'),
    ...
    getConnectionInfo: (id) => 
      ipcRenderer.invoke('get-connection-info', id),
    
    // Get instance logs
    getInstanceLogs: (id) => 
      ipcRenderer.invoke('get-instance-logs', id),
