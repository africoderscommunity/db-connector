...
export const DatabaseProvider = ({ children }) => {
  const { setShowAddForm, setEditingId, editingId ,addLog,connectionStatus, setConnectionStatus, } = useApp();
  ...
  const connectToDatabase = async (conn, loadTables = true) =>
    await dbService.connectToDatabase(conn, loadTables, { setLoading, setError, setActiveConnection, setTables, setConnectingId });

  const LoadDbTable =async  (tableConfig) => await dbService.LoadDbTable(tableConfig, { setTables });

  const testConnection = async () => await dbService.testConnection(formData, { setTestingConnection, setError });
  ...
  const addConnection = async() =>
    await dbService.addConnection({ formData, editingId, connections, saveConnections, setFormData, setShowAddForm, setEditingId, setError });
  const editConnection = async (conn) => await dbService.editConnection({setFormData,setShowAddForm , setEditingId, conn})
  const exportData = (data) =>dbService.exportData(selectedTable)(data)
  const loadTableData = async (tableName, options = {}) => await dbService.loadTableData(tableName, {
    ...options,
    activeConnection,
...
const executeQuery = (query,queryObject={}) =>dbService.executeQuery(query,queryObject={},{
  activeConnection,
    setError,
    setLoading,
    setTableData,
...
const loadSchemaMetadata = async (connectionId = activeConnection?.id) => {
    try {
      ...
      const cachedAutoCompletionData = connectionStatus?.[connectionId]?.autoCompletion
  ...
  return (
    <DatabaseContext.Provider value={{
      ...
      exportData,loadSchemaMetadata,
      loadTableData,loadConnections,
      deleteConnection,executeQuery,
      schemaMetadata, setSchemaMetadata
    }}>
      {children}
