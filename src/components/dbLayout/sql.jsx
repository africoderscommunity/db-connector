...
const MONGO_QUERIES = {
  'Read Operations': [
    ...
    {
      name: 'Find with AND',
      query: 'db.collection_name.find({"age": {"$gt": 18}, "status": "active"})',
      description: 'Multiple conditions (implicit AND)'
    },
    {
      name: 'Find with OR',
      query: 'db.collection_name.find({"$or": [{"age": {"$lt": 18}}, {"age": {"$gt": 65}}]})',
  ...
  'Write Operations': [
    {
      name: 'Insert One',
      query: 'db.collection_name.insertOne({"name": "John", "email": "john@example.com", "age": 30})',
      description: 'Insert a single document'
    },
    {
      name: 'Insert Many',
      query: 'db.collection_name.insertMany([{"name": "Alice", "age": 25}, {"name": "Bob", "age": 30}])',
      description: 'Insert multiple documents'
    },
    {
      name: 'Update One',
      query: 'db.collection_name.updateOne({"name": "John"}, {"$set": {"age": 31}})',
      description: 'Update first matching document'
    },
    {
      name: 'Update Many',
      query: 'db.collection_name.updateMany({"status": "pending"}, {"$set": {"status": "active"}})',
      description: 'Update all matching documents'
    },
    {
      name: 'Increment Value',
      query: 'db.collection_name.updateOne({"_id": "123"}, {"$inc": {"views": 1}})',
      description: 'Increment a numeric field'
    },
    {
      name: 'Push to Array',
      query: 'db.collection_name.updateOne({"_id": "123"}, {"$push": {"tags": "new-tag"}})',
      description: 'Add item to array field'
    },
    {
      name: 'Delete One',
      query: 'db.collection_name.deleteOne({"_id": "507f1f77bcf86cd799439011"})',
  ...
  'Aggregation': [
    ...
    {
      name: 'Group By',
      query: 'db.collection_name.aggregate([{"$group": {"_id": "$city", "count": {"$sum": 1}}}])',
      description: 'Group documents and count'
    },
    {
      name: 'Group with Sum',
      query: 'db.collection_name.aggregate([{"$group": {"_id": "$category", "total": {"$sum": "$price"}}}])',
      description: 'Group and sum values'
    },
    {
      name: 'Group with Average',
      query: 'db.collection_name.aggregate([{"$group": {"_id": "$department", "avgSalary": {"$avg": "$salary"}}}])',
      description: 'Group and calculate average'
    },
    {
      name: 'Match and Group',
      query: 'db.collection_name.aggregate([{"$match": {"status": "active"}}, {"$group": {"_id": "$city", "count": {"$sum": 1}}}])',
  ...
  'Advanced Queries': [
    ...
    {
      name: 'Regex Search',
      query: 'db.collection_name.find({"name": {"$regex": "^John", "$options": "i"}})',
    ...
    {
      name: 'Lookup (Join)',
      query: 'db.collection_name.aggregate([{"$lookup": {"from": "other_collection", "localField": "_id", "foreignField": "userId", "as": "user_data"}}])',
    ...
    <div className="bg-gray-800 border-r border-gray-700 w-80 overflow-y-auto">
      <div className="p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-semibold text-gray-300">MongoDB Query Helper</h2>
        </div>
              ...
              {expandedCategories[category] && (
                <div className="bg-gray-800 divide-y divide-gray-700">
                  {queries.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="p-3 hover:bg-gray-750 transition">
                        <div className="flex items-start justify-between mb-1">
                          <h4 className="text-sm font-medium text-blue-300">{item.name}</h4>
                          <button
                            onClick={() => handleCopyQuery(item.query, uniqueId)}
