...
import { Layers, ChevronRight, Copy, Pin, Download, Upload, Plus, FileCode, Filter, Trash2, X } from 'lucide-react';
import { useDatabase } from '../../context/DatabaseContext';
import { useAppContext } from '../../context/AppContext';
import  StructureModal  from './StructureModal';

export default function ActiveConnectionData() {
  const { setActiveTable, tables, LoadDbTable, activeConnection ,loadTableData,selectedTable } = useDatabase();
 ...
 const handleExport = (format) => {
  ...
  window.electron.db
    ...
    .then((res) => {
      if (res.success) {
        alert(`✅ Table '${exportTable}' exported successfully to ${res.filePath}`);
      } else {
  ...
  break;
      ...
      case 'truncate':
        if (
          confirm(`Are you sure you want to truncate ${table}? This will delete all data.`)
        ) {
          alert(`Truncating ${table}`);
        }
        break;
      case 'delete':
  if (confirm(`Are you sure you want to delete ${table}? This action cannot be undone.`)) {
    window.electron.db.deleteTable({
      connection: activeConnection,
      tableName: table,
    }).then((res) => {
      alert(res.message);
      if (res.success) {
        LoadDbTable({ connectionId: activeConnection.id, refresh: true });
      }
    });
  }
  break;
              ...
              .map((table) => (
                <div
                  ...
                  className={`p-2 mb-1 rounded cursor-pointer flex items-center gap-2 transition ${
                    selectedTable === table ? 'bg-blue-600' : 'hover:bg-gray-700'
                  }`}
                >
