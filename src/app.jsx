...
export default function DBConnector() {
  const { error, setError, activeConnection, tableData, loadConnections,setActiveConnection, setTables} = useDatabase();
  const { showAddForm, connectionStatus, setConnectionStatus, activeTab, setActiveTab } = useAppContext();
  ...
  useEffect(() => {
    window.electron.db.getSessionStatus().then((res) => {
      ...
      if (JSON.stringify(connectionStatus) !== JSON.stringify(res)) {
        ...
        const {config:cachedConnection, tables } = res[res?.currentActiveDatabase]
        console.log({tables})
        if(cachedConnection){
              ...
              <PanelGroup direction="vertical">
                ...
                <Panel defaultSize={20} minSize={10}>
                  <div className="h-full bg-gray-950 text-gray-300 p-3 font-mono text-sm overflow-auto border-t border-gray-800">
                    <h3 className="text-gray-400 text-xs mb-2 uppercase tracking-wide">Logs</h3>
                    <LogsPanel />
                  </div>
                </Panel>
              </PanelGroup>
